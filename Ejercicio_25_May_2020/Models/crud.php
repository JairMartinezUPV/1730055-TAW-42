<?php
	...
	class Datos extends Conexion {
		...
		public function vistaHistoriaModel($tabla)
		{
			$stmt = Conexion::conectar()->prepare("SELECT CONCAT(u.firstname,':',u.user_name) AS 'usuario', p.name_product AS 'producto',h.date AS 'fecha', h.reference AS 'referencia', h.note AS 'nota', h.quantity AS 'cantidad', FROM $tabla h INNER JOIN products p ON h.id_producto = p.id_producto INNER JOIN users u ON h.user_id = u.user_id");
			$stmt->execute();
			return $stmt
		}
